...
def footer(c, page_no, total=None):
    ...
    B.draw_bulb(c, MARGIN + 5, 28, 5, body=B.YELLOW)
    c.setFont(B.BOLD, 8)
    c.setFillColor(B.INK)
    c.drawString(MARGIN + 16, 25, 'divergenios.com')
...
def page_cover(c):
    ...
    # mascota grande
    B.draw_mascot(c, W / 2, H * 0.72, 70, body=B.YELLOW)
    # titulo
    c.setFillColor(white)
    c.setFont(BOLD, 46)
...
def page_welcome(c):
    ...
    B.draw_mascot(c, MARGIN + 50, H - 80, 36)
    c.setFillColor(B.INK)
    c.setFont(BOLD, 26)
...
def page_notebook(c, page_no):
    ...
    B.draw_mascot(c, W - MARGIN - 34, H - 44, 26)
    c.setFillColor(white)
    c.setFont(BOLD, 24)
...
def page_back(c, page_no):
    ...
    c.rect(0, 0, W, H, fill=1, stroke=0)
    B.draw_doodles(c, W, H, color=white)
    B.draw_mascot(c, W / 2, H * 0.74, 56, body=B.YELLOW)
    c.setFillColor(white)
    c.setFont(BOLD, 30)
